#!/usr/bin/env python3
"""
Teste real do Cloud Storage Failover com Cloudflare R2.

Este teste:
1. Provisiona GPU em qualquer regiao
2. Monta bucket R2 via rclone
...
R2_ACCOUNT_ID = "142ed673a5cc1a9e91519c099af3d791"
R2_ACCESS_KEY_ID = "f0a6f424064e46c903c76a447f5e73d2"
R2_SECRET_ACCESS_KEY = "1dcf325fe8556fca221cf8b383e277e7af6660a246148d5e11e4fc67e822c9b5"
R2_BUCKET = os.environ.get("R2_BUCKET", "dumont-snapshots")
...
async def setup_rclone_r2(ssh_host, ssh_port, timeout=180):
    """Configura rclone e monta bucket R2"""
    import subprocess

    # Script para configurar R2 com rclone
...
echo "=== Configurando rclone para Cloudflare R2 ==="
mkdir -p ~/.config/rclone

# Configurar rclone para R2 (S3 compatible)
...
echo "=== Testando conexao com R2 ==="
rclone lsd r2: 2>&1 | head -5

echo "=== Montando {R2_BUCKET} em /data ==="
mkdir -p /data
mkdir -p /tmp/rclone-cache

# Tentar montar
rclone mount r2:{R2_BUCKET} /data \\
    --vfs-cache-mode full \\
...
if mountpoint -q /data 2>/dev/null; then
    ...
    rclone lsd r2:{R2_BUCKET} 2>/dev/null | head -5
    echo "RCLONE_OK"
fi
'''
...
async def main():
    print("=" * 60)
    print("TESTE REAL - CLOUD STORAGE FAILOVER COM CLOUDFLARE R2")
    print("=" * 60)
    print()
    print(f"R2 Bucket: {R2_BUCKET}")
    print(f"R2 Account: {R2_ACCOUNT_ID[:8]}...")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # 5. Configurar R2 storage
        print("4. Configurando Cloudflare R2...")
        storage_start = time.time()

        print("   Aguardando SSH...")
        await asyncio.sleep(10)

        success, output = await setup_rclone_r2(ssh_host, ssh_port)
        storage_time = time.time() - storage_start

        if success:
            print(f"   R2 CONFIGURADO!")
        ...
        print("=" * 60)
        print("RESULTADO DO TESTE - CLOUD STORAGE FAILOVER (R2)")
        print("=" * 60)
        print()
        print(f"  GPU: {used_gpu['gpu_name']}")
        print(f"  Regiao: {used_gpu['geo']}")
        ...
        print(f"    - R2 Storage:    {storage_time:.1f}s")
        ...
        print("  - Cloud Storage GCS:  ~53s  (QUALQUER regiao)")
        print(f"  - Cloud Storage R2:   ~{total_time:.0f}s  (QUALQUER regiao) [SEM EGRESS]")
